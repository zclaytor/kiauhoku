...
def my_RGBTip(track, eep_params, i0=None):
    '''
    ...
    if RGBTip == i0 or lum_tr.loc[RGBTip] < 2: # one track misIDs the TRGB too low
        return -1
    return RGBTip
...
def install(
    ...
    eep_functions={'rgbump': 'skip', 'rgbtip': my_RGBTip},
    ...
    with open(os.path.join(path, 'yrec_eep_params.pkl'), 'wb') as f:
    ...
    full_save_path = os.path.join(path, 'yrec_full.pqt')
    ...
    eep_save_path = os.path.join(path, 'yrec_eep.pqt')
    ...
    interp_save_path = os.path.join(path, 'yrec_interpolator.pkl')
